...
contract GMAPools is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (remaining > 0) {
            if (poolReward.add(poolReward.div(10)) < remaining) {
                GMA.mint(devaddr, poolReward.div(10));
                GMA.mint(address(this), poolReward);
                ...
                GMA.mint(devaddr, devReward);
                GMA.mint(address(this), remaining.sub(devReward));
    ...
    function _safeRewardTransfer(address _to, uint256 _amount) internal {
        uint256 rewardBal = GMA.balanceOf(address(this));
        if (_amount > rewardBal) {
            IERC20(GMA).safeTransfer(_to, rewardBal);
        } else {
            IERC20(GMA).safeTransfer(_to, _amount);
        }
    }
    ...
    function batchAddPools(IERC20[] calldata _stakedTokens, uint256[] calldata _allocPoints, POOL_TYPE[] calldata _poolTypes, bool _withUpdate) external onlyOwner {
        require(_stakedTokens.length == _allocPoints.length && _stakedTokens.length == _poolTypes.length, "GMAPools: Invalid length of pools");
    ...
    function batchSetPoolsByStakedToken(IERC20[] calldata _stakedTokens, uint256[] calldata _allocPoints, bool _withUpdate) external onlyOwner {
        require(_stakedTokens.length == _allocPoints.length, "GMAPools: Invalid length of pools");
        for(uint i = 0; i < _stakedTokens.length; i++) {
