...
contract GMABoardroom is Ownable, ReentrancyGuard {
    ...
    modifier onlySetter() {
        require(isSetter(msg.sender), "GMABoardroom: Not the setter");
        _;
    }
    ...
    function getSetter(uint256 _index) public view returns (address){
        require(_index <= getSetterLength() - 1, "GMABoardroom: index out of bounds");
    ...
    function _withdraw(uint256 _lockedId, bool _withReward) internal nonReentrant {
        ...
        if (_amount > 0) {
            require(!user.lockedInfo[_lockedId].isWithdrawed, "GMABoardroom: This amount of lockedId is withdrawed");
            ...
            if (expireTime < block.timestamp) {
                ...
                if (interval <= minLockTime) {
                    require(maxPenaltyOn, "GMABoardroom: maxLockTime");
                    penlaty = _amount.mul(maxPenalty).div(100);
                } else if (interval <= midLockTime) {
                    require(midPenaltyOn, "GMABoardroom: midLockTime");
                    penlaty = _amount.mul(midPenalty).div(100);
                } else {
                    require(minPenaltyOn, "GMABoardroom: minLockTime");
    ...
    function setPenaltyPencentage(uint256 _min, uint256 _mid, uint256 _max) external onlyOwner {
        require(_min <= _mid && _mid <= _max && _max <= 100, "GMABoardroom: invalid penalty percentage");
        minPenalty = _min;
        midPenalty = _mid;
        maxPenalty = _max;
    }
    ...
    function addRewardToken(uint256 _startTime, address _rewardToken, uint256 _decimals, string memory _symbol) external onlySetter {
        require(_startTime > block.timestamp, "GMABoardroom: invalid start time");
        require(_rewardToken != address(poolInfo.stakedToken), "GMABoardroom: staked token is reward token");
        require(!isExistedRewardToken[_rewardToken], "GMABoardroom: existed reward token");
        require(rewardTokenInfo.length < MAX_REWOARD_TOKEN, "GMABoardroom: max reward token");
        massUpdatePools();
    ...
    function depoistRewardToken(uint256 _amount, uint256 _rid) onlySetter external {
        require(setterOfRid[_rid] == msg.sender, "GMABoardroom: incorrect setter of this reward token pool");
        require(_amount > 0, "GMABoardroom: zero amount");
        massUpdatePools();
        RewardTokenInfo storage token = rewardTokenInfo[_rid];
        ...
        require(currBal.sub(prevBal) == _amount.mul(amountUnit), "GMABoardroom: incorrect balance after depositing");
